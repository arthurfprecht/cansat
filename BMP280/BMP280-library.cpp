...
#include "BMP280-library.h"
#include "..\Wire\Wire.h" // for I2C protocol

// *****************************************************
// constructors
// the constructors set up the I2C address and debug printing control
bme280::bme280 (uint8_t Addr, uint8_t debug) {       // constructor for class bme280
  address = bmp280Addr + Addr;
  bmp280Debug = debug;
} // end of bme280 constructor

bme280::bme280 (uint8_t Addr) {                    // bme280 constructor with 1 parameter
  address = bmp280Addr + Addr;
  bmp280Debug = 0;
}
...
bmp280::bmp280 (uint8_t Addr, uint8_t debug) { // constructor for class bmp280
  address = bmp280Addr + Addr;
  bmp280Debug = debug;
} // end of bmp280 constructor

bmp280::bmp280 (uint8_t Addr) {             // constructor with 1 parameter
  address = bmp280Addr + Addr;
  bmp280Debug = 0;
}
...
bool bmp280::begin(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode, uint8_t t_sb, uint8_t filter, uint8_t spi3W_en) {
  ...
  */
  uint8_t ID = readRegister(0xD0); // chip-ID is in register 0xD0
  /*
  ...
  */
  ...
  return true;
} // end of void bmp280::begin(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode, uint8_t t_sb, uint8_t filter, uint8_t spi3W_en)

bool bme280::begin(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode, uint8_t t_sb, uint8_t filter, uint8_t spi3W_en, uint8_t osrs_h) {
  ...
  */
  ...
  updateF5Config(t_sb, filter, spi3W_en); // standby time, IIR filter, I2C select
  return true;
} // end of void bme280::begin(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode, uint8_t t_sb, uint8_t filter, uint8_t spi3W_en, uint8_t osrs_h)
...
double bmp280::readTemperature () {
  ...
  uint8_t data[3]; // array of 3 uint8_ts used to store temperature data
  uint8_t errorCode = readByteArray(0xFA, 3, data); // get 3 uint8_ts from 0xFA (temperature)
  // Convert temperature data uint8_ts to 20-bits within 32 bit integer
...
long bmp280::readRawPressure (long &adc_t) {
  ...
  uint8_t data[6]; // array of 6 uint8_ts used to store pressure and temperature data
  uint8_t errorCode = readByteArray(0xF7, 6, data); // get 6 uint8_ts from 0xF7 (pressure and temperature)
...
long bme280::readRawHumidity (long &rawTemperature, long &rawPressure) { // reads raw humidity, temperature and pressure
  ...
  uint8_t data[8]; // array of 8 uint8_ts used to store pressure and temperature data
  uint8_t errorCode = readByteArray(0xF7, 8, data); // get 8 uint8_ts from 0xF7 (pressure, temperature, humidity)
  // Convert pressure and temperature data to 19-bits
...
void bme280::updateF2Control(uint8_t osrs_h) {
  ...
  uint8_t F2value = (osrs_h & 0x7);   // construct bit pattern
  updateRegister(0xF2, F2value);
}  // end of void updateF2Control(uint8_t osrs_h)

uint8_t bmp280::updateF4Control(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode) {
  ...
  uint8_t F4value = (osrs_t & 0x7) << 5 | (osrs_p & 0x7) << 2 | (mode & 0x3); // construct bit pattern
  uint8_t error = updateRegister(0xF4, F4value);
  return error;
}  // end of void updateF4Control(uint8_t osrs_t, uint8_t osrs_p, uint8_t mode)

uint8_t bmp280::updateF4Control16xNormal() {   // function
  uint8_t error = updateF4Control(7, 7, 3); // temperature 20 bit, pressure 20 bit, mode Normal (continuous)
  return error;
} // end of  void bmp280::updateF4Control16xNormal()

uint8_t bmp280::updateF4Control16xSleep() {   // function
  uint8_t error = updateF4Control(7, 7, 0); // temperature 20 bit, pressure 20 bit, mode Sleep
  return error;
} // end of  void bmp280::updateF4Control16xSleep()

uint8_t bmp280::updateF4ControlSleep(uint8_t value) {
  ...
  uint8_t error = updateF4Control(value >> 3 & 0x07, value & 0x07, 0); // Sleep mode and 6 bits of value
  return error;
} // end of void bmp280::updateF4ControlSleep(uint8_t value)

uint8_t bmp280::updateF5Config(uint8_t t_sb, uint8_t filter, uint8_t spi3W_en) {   // function
  ...
  uint8_t F5value = (t_sb & 0x07) << 5 | (filter & 0x07) << 2 | (spi3W_en & 0x03); // construct bit pattern
  uint8_t error = updateRegister(0xF5, F5value);
  return error;
} // end of void bmp280::updateF5Config(uint8_t t_sb, uint8_t filter, uint8_t spi3W_en)

uint8_t bmp280::updateF5Config1msIIR16I2C() {   // function
  uint8_t error = updateF5Config(0, 0, 0); // standby time 0.5ms, filter Off, I2C
  return error;
} // end of void bmp280::updateF5Contfig1msIIR16I2C()

uint8_t bmp280::updateF5ConfigSleep(uint8_t value) {
  ...
  uint8_t error = updateF5Config(value >> 3 & 0x07, value & 0x07, 0); // I2C and 6 bits of value
  return error;
} // end of void bmp280::updateF5ConfigSleep(uint8_t value)

// ********************************************
// calibration functions
void bmp280::getBmpCalibratonData() { // function
  // get calibration data
  uint8_t b1[24]; // array of 24 uint8_ts used to store calibration uint8_ts.
  uint8_t errorCode = readByteArray(0x88, 24, b1); // 0x88 Hex is start address of calibration data
...
void bme280::getBmeCalibratonData() { // function
  // get calibration data
  dig_H1 = readRegister(0xA1); // dig_H1 is a single uint8_t separated from the others

  // next read the 7 uint8_ts of contiguous humidity calibration data
  uint8_t b1[7]; // array of 7 uint8_ts used to store humidity calibration uint8_ts.
  uint8_t errorCode = readByteArray(0xE1, 7, b1);
...
uint8_t bmp280::readRegister(uint8_t reg) {
  ...
  */
  return (data);
}  // end of void readRegister(uint8_t reg)

uint8_t bmp280::updateRegister(uint8_t reg, uint8_t value) {   // function
  noInterrupts(); // disable interrupts
  ...
  uint8_t error = Wire.endTransmission(); // end of write
  interrupts(); // enable interrupts
  /*
  if (error && bmp280Debug) {
    Serial.print ("I2C error with address ");
  ...
  */
  return error;
} // end of void bmp280::updateRegister(uint8_t reg, uint8_t value)

uint8_t bmp280::readByteArray(uint8_t reg, uint8_t length, uint8_t data[]) { // function
  ...
  */
  ...
  // Request "length" uint8_ts of data
  Wire.requestFrom(address, length);
  uint8_t uint8_tCount = Wire.available();
  if (uint8_tCount == length)   { // correct answer
    /*
    if (bmp280Debug) {
      Serial.print ("\n(debug) getting ");
      Serial.print (uint8_tCount);
      Serial.println (" uint8_ts");
    } // end of if (bmp280Debug)
    */
  } else {                    // uint8_tCount does not match length
    /*
    if (bmp280Debug) {
      Serial.print ("uint8_tCount not correct, but is ");
      Serial.println (uint8_tCount);
    } // end of if (bmp280Debug)
    */
    return 100 + uint8_tCount; // 100 offset to avoid confusion with error code
  } // end of if uint8_tcount == length)

  for (int i = 0; i < length; i++)  { // reads "length" uint8_ts
    data[i] = Wire.read();
